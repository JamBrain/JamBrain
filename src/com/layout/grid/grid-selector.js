...
export default class GridSelector extends Component {
  ...
  render( props, {expanded, selected} ) {
    ...
    if (expanded) {
      const options = [
        [
          1, '1 per line', < div onclick = {
            () => this.onSelectLayout(1)
          }
          class = "-click-catcher" />
        ],
        [
          1, '2 per line', < div onclick = {
            () => this.onSelectLayout(2)
          }
          class = "-click-catcher" />
        ],
        [
          1, '3 per line', < div onclick = {
            () => this.onSelectLayout(3)
          }
          class = "-click-catcher" />
        ],
        [
          1, '4 per line', < div onclick = {
            () => this.onSelectLayout(4)
          }
          class = "-click-catcher" />
        ],
        [
          1, '5 per line', < div onclick = {
            () => this.onSelectLayout(5)
          }
          class = "-click-catcher" />
        ],
        [
          1, '6 per line', < div onclick = {
            () => this.onSelectLayout(6)
          }
          class = "-click-catcher" />
        ]
      ];

      ShowDropDown = (<Dropdown items={options} onmodify={this.onSelectLayout} startExpanded={true} value={selected} selfManaged={false}/>);
    }
