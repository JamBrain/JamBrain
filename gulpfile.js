...
//gulp.task('js-cat', gulp.series(['buble-rollup','js'], function() {
...
gulp.task('git-version.php', function() {
	var fs = require('fs');
	require('shelljs/global');
	
	var Hash = exec('git describe --always',{silent:true}).stdout.trim();
	var Commit = exec('git rev-list HEAD | wc -l',{silent:true}).stdout.trim();
	
	fs.writeFileSync('output/git-version.php', "<?php\n// WARNING! DO NOT MODIFY! This file is automatically generated by gulpfile.js!\n\nconst GIT_VERSION = '"+Commit+"-"+Hash+"';\n");
});
...
//				.pipe( gulp.symlink( release_folder+'/'+js_output ) );	
	...
	if ( glob.sync('output/git-version.php').length === 0 ) {
		gulp.start('git-version.php');
	}
